...
Open `index.html` for a filterable gallery with arch and square previews. `web/` contains 768px optimized WebP assets; `originals/` preserves the generated square PNGs. `manifest.json` records the exact prompts and the signature-removal edit. `baseball-revision.json` documents the revised natural pitching scene; its earlier composition is retained in `sources/vampire-baseball-v1.png`. The share ZIP contains the gallery, web assets, contact sheet, and manifest; originals remain in this folder.
